...
def infer_task_type(files: dict[str, Any]) -> tuple[str, list[str], str]:
    ...
    if has_choices:
        return "multiple_choice", ["text"] + (["image"] if has_image else []), "choice"
    if has_question:
        return "text_qa", ["text"] + (["image"] if has_image else []), "text"
    if files.get("sample_submission", {}).get("exists"):
        return "classification", ["table"], "label"
    return "unknown", [], "unknown"


def infer_id_target_columns(sample_columns: list[str]) -> tuple[str, str]:
    if not sample_columns:
        return "unknown", "unknown"
    id_candidates = [c for c in sample_columns if c.lower() in {"id", "index", "sample_id", "uid"}]
    id_col = id_candidates[0] if id_candidates else sample_columns[0]
